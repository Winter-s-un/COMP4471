...
def svm_loss(x, y):
    ...
    # scores = x.dot(x)
    x = x.T
    correct_class_scores = x[y, np.arange(num_train)]


    margins = np.maximum(0, x - correct_class_scores + 1)
    margins[y, np.arange(num_train)] = 0  # for j = y

    loss = np.sum(margins) / num_train
    margins[margins > 0] = 1
    margins[y, np.arange(num_train)] = -np.sum(margins, axis = 0)

    dx = margins.T / num_train
...
def softmax_loss(x, y):
    ...
    # loss /= N
    #
    # weights = exp_scores / np.sum(exp_scores, axis = 1).reshape(-1, 1)
